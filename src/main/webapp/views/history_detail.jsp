    ...
    <div class="space-y-6">
        <c:forEach var="item" items="${historyDetails}" varStatus="status">
            <div class="relative rounded-2xl border p-6 transition-all ${item.isCorrect ? 'border-slate-100 bg-white' : 'border-red-100 bg-red-50/30'}">
                ...
                <p class="mb-6 text-lg font-bold text-slate-900 pr-24">Q. ${fn:escapeXml(item.questionText)}</p>
                <div class="grid gap-4 sm:grid-cols-2">
                    <div class="rounded-xl p-4 ${item.isCorrect ? 'bg-green-50 border border-green-100' : 'bg-white border border-red-100'}">
                        <span class="block text-xs font-bold ${item.isCorrect ? 'text-green-500' : 'text-slate-400'} mb-1">내가 쓴 답</span>
                    ...
                    <div class="rounded-xl bg-blue-50 p-4 border border-blue-100">
                        ...
                        <span class="font-bold text-blue-700">${fn:escapeXml(item.answerText)}</span>
                    </div>
                </div>
                <c:if test="${not empty item.explanation}">
                    <div class="mt-4 pt-4 border-t border-slate-200/50 text-sm text-slate-600">
                        <span class="font-bold mr-2">💡 해설:</span> ${fn:escapeXml(item.explanation)}
                    </div>
                </c:if>
            </div>
        </c:forEach>
